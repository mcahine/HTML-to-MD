...
class SspaiConverter(HTMLConverterBase if HAS_BASE else object):
    ...
    def __init__(self):
        if HAS_BASE:
            super().__init__(
                domain='sspai.com',
                name='少数派',
                content_selector='.article__main__content, article, .content',
                title_selector='h1',
                author_selector='.ss__user__nickname',
                date_selector='.timer',
                remove_selectors=[
                    '.sidebar', '.article-meta', '.author-card',
                    '.comment-section', '.related-articles', '.tag-list',
                    '.article-author', '.author-box', '.article-header-author',
                    '.author-popover', '.el-popover', '.el-popper',
                    '.author-item', '.follow-btn', '.author-profile',
                    '.article-footer', '.article-copyright', '.article-recommend',
                    '.comment-area', '.comments-section', '.paywall',
                    '.matrix-recommend', '.article-sponsor',
                    '.toc', '.catalog', '.outline', '.table-of-contents',
                    '.comment-list', '.comment-form', '.discussion',
                    '.editor-info', '.article-editor', '.responsible-editor',
                    '.article-actions', '.article-share', '.article-toolbar',
                    '.read-more', '.related-posts', '.post-navigation',
                    'header', '.ss__custom__header', '.ss__custom__footer',
                ]
            )
        else:
    ...
    def extract_metadata(self, soup, html_path):
        ...
        # 如果还没找到 URL，从 HTML 注释中提取 (SingleFile 格式)
        if not metadata['url']:
            html_content = str(soup)
            # 匹配 SingleFile 注释中的 url
    ...
    def convert_images(self, content, output_dir):
        ...
        img_dir = os.path.join(output_dir, 'assets', f'{stem}_assets') if stem else os.path.join(output_dir, 'assets')
                ...
                try:
                    ...
                    if match:
                        ...
                        try:
                            ...
                            # 更新 img 标签的 src
                            rel_path = os.path.join('assets', f'{stem}_assets', img_name) if stem else os.path.join('assets', img_name)
                            img['src'] = rel_path
    ...
    def convert(self, html_path, output_dir=None):
        ...
        # 处理图片
        img_count = self.stem = Path(html_path).stem
        ...
        yaml_front += f"source: 少数派\n"
        yaml_front += f"url: {metadata.get('url', metadata['source'])}\n"
    ...
    def post_process_markdown(self, md_content, metadata):
        ...
        # 清理卡片链接中的图片（如 [文本 ![](图片)](链接) -> [文本](链接)）
        # 匹配模式：[ 任意文本 ![](图片路径) 任意文本 ](链接)
        md_content = re.sub(
            r'\[([^\]]*?)!\[.*?\]\([^\)]+\)([^\]]*?)\]\(([^\)]+)\)',
            r'[\1\2](\3)',
            md_content
        ...
        # 清理卡片链接中的多余空格和 "__ sspai.com" 后缀
        # 例如：[ 文本 __ sspai.com ](链接) -> [文本](链接)
        md_content = re.sub(
            r'\[\s*([^\]]*?)\s*__\s*sspai\.com\s*\]\(([^\)]+)\)',
            r'[\1](\2)',
            md_content
        )
        # 清理链接文本中的多余空格
