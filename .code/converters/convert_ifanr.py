#!/usr/bin/env python3
"""爱范儿 (ifanr.com) HTML 转 Markdown 转换器"""
import sys
import os
import re
import base64
...
class IfanrConverter(HTMLConverterBase):
    def __init__(self):
        super().__init__(
            domain='ifanr.com',
            name='爱范儿',
    ...
    def extract_metadata(self, html_content, html_path):
        ...
        for sel in ['.author-name', '.article-author', '.author', '[class*="author"]']:
            ...
            if author_elem:
                ...
                # 清理过长的作者信息（爱范儿有时会包含简介）
                if author_text:
                    # 只取前20个字符作为作者名
                    author_text = author_text[:30]
                    # 如果包含常见分隔符，只取第一部分
                    for sep in ['发邮件', '科技编辑', '累计已', '最近文章']:
                        if sep in author_text:
                            author_text = author_text.split(sep)[0].strip()
                            break
    ...
    def extract_content(self, soup):
        ...
        # 爱范儿备用模板: .page-body / .o-single-content__body
        for fallback in ['.page-body', '.o-single-content__body', '.post-content',
    ...
    def convert(self, html_path, output_dir=None):
        ...
        # 固定输出目录
        domain_dir = os.path.join(output_dir, '爱范儿')
...
if __name__ == '__main__':
    converter = IfanrConverter()
    if len(sys.argv) > 1:
        if sys.argv[1].endswith('.html'):
            converter.convert(sys.argv[1])
        else:
            import glob
            files = glob.glob(os.path.join(sys.argv[1], "*爱范儿*.html"))
            print(f"Found {len(files)} files")
            for f in files:
                converter.convert(f)
    else:
        import glob
        files = glob.glob("*爱范儿*.html")
        print(f"Found {len(files)} files")
        for f in files:
            converter.convert(f)
